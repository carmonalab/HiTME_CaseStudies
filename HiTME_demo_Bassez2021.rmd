...
[Demo dataset link](https://figshare.com/account/projects/190248/articles/24916611)
...
Additional signatures not included in the models (e.g. cell cycling, IFN/HSP response...) can also be evaluated using `additional.signatures` parameter. These are lists of defining genes for these signatures manually customed or retrieved from [SignatuR](https://github.com/carmonalab/SignatuR).
...
* Cell relative proportions (`freq`)
    
* Centered log ratio (CLR)-transformed cell counts (`freq_clr`)
...
#### Celltype frequency plots {#freq-plots}
...
The parameter `by.x` defines which variables, sample or celltype, are included in the x-axis the plot types, rendering a barplot or boxplot, respectively.
...
We can explore this further but checking the markers on these myeloid cells.
...
As we have seen, [HiTME](https://github.com/carmonalab/HiTME) and authors classification show high agreement on T cell classification (~80%). However, authors do not tell apart CD4+ and CD8+ T cells, and do not take into account NK cells.
